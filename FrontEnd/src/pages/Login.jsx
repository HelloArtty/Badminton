...
function Login() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const result = await AxiosLib.post("/backend/auth/log-in", {
        email: login.email,
        ...
        <div className="bg-emerald-600 min-h-screen w-1/2 p-8 hidden md:block">
          <div className="p-8 min-h-screen items-center justify-center flex">
            <div className="max-w-md p-8 text-center">
              <h2 className="text-2xl text-gray-100 mb-2">Welcome back to</h2>
              <h1 className="text-8xl text-white font-bold mb-3">BadLink</h1>
              <h3 className="text-sm text-gray-200  mb-2">
                Welcome back again, we are here to help you. Log in now to book
                your badminton court and continue your winning streak. Let's
                make every game count! together.
              </h3>
            </div>
          </div>
        </div>
